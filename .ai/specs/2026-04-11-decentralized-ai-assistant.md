...
export async function loadAllModuleTools(): Promise<void> {
  ...
  try {
    const { aiToolConfigEntries } =
      await import("@/.mercato/generated/ai-tools.generated");
    for (const entry of aiToolConfigEntries) {
      loadModuleTools(entry.moduleId, entry.tools as ModuleAiTool[]);
    }
    console.error(
      `[MCP Tools] Loaded tools from ${aiToolConfigEntries.length} modules`
    );
  } catch (error) {
...
import type {
  ...
  AiManifest,
  AiContextDependency,
} from "@open-mercato/shared/modules/ai";
...
- Role: ${ctx.isSuperAdmin ? "Super Admin" : "Standard User"}`);
  ...
  const moduleList = activeManifests
    .map(
      (e) =>
        `- **${e.manifest.domain}** (${e.moduleId}): ${e.manifest.description}`
    )
    .join("\n");
...
export const aiManifest: AiManifest = {
  domain: "Search",
  description:
    "Hybrid search across all business data — customers, products, orders, deals, and more",
  keywords: [
    "search",
    "find",
    "lookup",
  ...
  dataCapabilities: [
    {
      entity: "any",
      description:
        "Cross-entity search with fulltext, vector, and token strategies",
...
export const aiManifest: AiManifest = {
  domain: "CRM",
  description:
    "Customer relationship management — companies, people, deals, and activities",
  keywords: [
    "customer",
    "company",
    "person",
...
export const aiManifest: AiManifest = {
  ...
  contextDependencies: [
    {
      moduleId: "customers",
      reason: "Sales documents reference companies/people",
      required: true,
    },
    {
      moduleId: "catalog",
      reason: "Line items reference products for pricing",
      required: true,
    },
    {
      moduleId: "search",
      reason: "Finding customers and products by name",
      required: false,
    },
  ],
...
if (context?.module && context?.recordId) {
  const moduleManifest = routeResult.manifests.find(
    (e) => e.moduleId === context.module
  );
...
export const aiManifest: AiManifest = {
  ...
  contextProvider: {
      ...
      if (entityType === "company") {
        ...
        return company
          ? {
              ...
              email: company.email,
              phone: company.phone,
              primaryContact: company.primaryContact?.displayName,
            }
          : null;
      }
    ...
    searchEntities: async (entityType, query, ctx) => {
      const searchService =
        ctx.container.resolve<SearchService>("searchService");
      ...
      return results.map((r) => ({
        id: r.recordId,
        label: r.presenter?.title ?? r.recordId,
        score: r.score,
      }));
    },
  },
...
async function createQuoteTool(input: CreateQuoteInput, ctx: AiToolContext) {
  ...
  const customer = await customersManifest?.manifest.contextProvider?.getEntity(
    "company",
    input.customerId,
    ctx
  );
...
const isAgentMode = agentConfig?.triggerPatterns?.some((pattern) =>
...
export const aiManifest: AiManifest = {
  ...
  apiWhitelist: [
    ...
    {
      method: "GET",
      path: "/api/sales/quotes/:id",
      description: "Get quote details with line items",
    },
    {
      method: "POST",
      path: "/api/sales/quotes",
...
export async function generateWhitelistTools(
  ...
  for (const entry of whitelist) {
    const endpoint = spec.describeEndpoint(
      entry.path,
    ...
    const fields = endpoint.requiredFields
      ...
      .filter(
        (f) => !entry.includeFields || entry.includeFields.includes(f.name)
      );
    ...
    const toolName =
      entry.toolName ??
      `${moduleId}_${entry.method.toLowerCase()}_${pathToName(entry.path)}`;
...
const filteredTools = mcpTools.filter(
  (t) =>
    ...
    t.name === "context_whoami" ||
    t.name === "search" ||
    t.name === "execute" // Code Mode fallback
);
```
  ...
  for (const entry of legacyTools) {
    if (!manifestModules.has(entry.moduleId)) {
      manifests.push({
        moduleId: entry.moduleId,
        manifest: {
          domain: entry.moduleId,
          description: "",
          keywords: [entry.moduleId],
          dataCapabilities: [],
          tools: entry.tools,
        },
      });
