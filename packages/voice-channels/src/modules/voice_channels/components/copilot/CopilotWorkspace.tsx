...
export function CopilotWorkspace() {
    ...
    (event) => {
      ...
      setSuggestions((prev) => {
        const filtered = prev.filter(
          (suggestion) =>
            suggestion.type !== card.type ||
            Date.now() - suggestion.createdAt < 60_000
        );
  ...
  const footerLabel = callInfo
    ? t(
        "voice_channels.copilot.workspace.footerSession",
        "Session: {callId}{providerPart}{providerCallPart}",
        {
          callId: callInfo.callId,
          providerPart: callInfo.providerKey
            ? t(
                "voice_channels.copilot.workspace.footerProvider",
                " · Provider: {providerKey}",
                {
                  providerKey: callInfo.providerKey,
                }
              )
            : "",
          providerCallPart: callInfo.providerCallId
            ? t(
                "voice_channels.copilot.workspace.footerProviderCall",
                " · Provider call: {providerCallId}",
                {
                  providerCallId: callInfo.providerCallId,
                }
              )
            : "",
        }
    ...
    : t(
        "voice_channels.copilot.workspace.footerWaiting",
        "Waiting for an active call session"
      );
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 gap-px overflow-hidden border-y bg-border">
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-card">
          <div className="border-b px-5 py-4 text-sm font-semibold text-muted-foreground">
            <span className="leading-6">
              {t(
                "voice_channels.copilot.workspace.transcriptTitle",
                "Live transcript"
              )}
            </span>
          </div>
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-muted/30">
          <div className="flex items-center justify-between border-b px-5 py-4 text-sm font-semibold text-muted-foreground">
            <span>
              🤖{" "}
              {t("voice_channels.copilot.workspace.panelTitle", "Call Copilot")}
            </span>
            <CopilotToggle
          ...
          {intentToast ? (
            ...
            >
              ...
              />
              {t(
                "voice_channels.copilot.workspace.intentDetected",
      ...
      <div className="flex items-center justify-between gap-3 border-t bg-card px-5 py-3">
        <span className="text-xs text-muted-foreground">
          {t(
            "voice_channels.copilot.workspace.footerCounts",
            "{segments} segments · {suggestions} suggestions",
            {
