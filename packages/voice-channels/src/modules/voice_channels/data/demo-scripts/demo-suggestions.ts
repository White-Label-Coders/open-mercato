...
/**
 * Synthetic suggestion cards keyed by the transcript segmentId that should trigger them.
 * Used by the standalone `?mock=local` / hackathon demo replay to showcase all 5 card
 * types without the Sub-Spec A orchestrator being online.
 *
 * The `id`, `triggerSegmentId`, `triggerText`, and `createdAt` fields are filled in
 * at dispatch time by the page's replay driver — the values below are placeholders.
 */
export const DEMO_SUGGESTIONS: Record<number, SuggestionCard> = {
